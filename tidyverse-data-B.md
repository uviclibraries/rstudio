...
While we may only want to handle some information about all of the item
in our data set, we may in turn want to select some items in our data
set fulfilling certain criteria.

- E.g. if you have a data set of names, you might want to select all
  names that begin with ‘A’.

To select items (rows, *not* columns), we use the `filter()`</code>\`
function.
...
- Pass the condition in as the parameter for the `mutate` function
  ...
  - If we want to add a variable (column), begin the parameter of
    ...
    - The new column name is `Low_Priority`
