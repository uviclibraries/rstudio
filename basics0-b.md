2-Getting Started for Mere Mortals
================
DSC
2024-02-07
...
the new variable - followed by: - an assignment operator `<-`, - and
then the data or expression that defines the content of the variable. -
...
specific arguments, if required, to produce a result. <br> -E.g.,
help(“integer”) - This calls the ‘help’ function with the argument (aka
parameter) “integer” - It will return information about an ‘integer’
object type.
...
- It’s used to create a vector by grouping individual values into a
  ...
  - Think of it as placing items into a container where each item
    ...
    - For example, `vector1 <- c(value1, value2)` creates a vector named
      ‘vector1’ containing the elements ‘value1’ and ‘value2’ as
      separate items in a sequence, not as a single merged item.
...
(top right window) under <u>Values</u>.<br>
...
<br> \#### <u>Task 2.2.4:</u> Make a vector for the following name
...
will create a matrix using the `matrix()` function. - For more on
matrices, [check me
out](https://www.w3schools.com/r/r_matrices.asp){:target=“\_blank”}. -
Instead of creating our own matrices, we will be importing data later
on.
...
median, standard deviation) are quite intuitive. - It is just the name
or abbreviation of the measure, - where the argument is the object
...
large standard deviation suggests a wide range of weights. <br>
...
![](basics0_files/figure-gfm/unnamed-chunk-19-1.png)<!-- -->
...
Multiple parameters: `function_name(parameter1, parameter2)` E.g.
...
![](basics0_files/figure-gfm/unnamed-chunk-20-1.png)<!-- -->
...
- ?? In **Yes** to install the “**readxl**” package.
  ...
  *Note:* Don’t worry about making a mistake importing this data. You
  can always remove it using the `rm()` function.
...
and 4 variables (columns) - It can be inferred that this data relates to
10 people - The values with each person are: - id (in lieu of a name)
(dbl) - gender (char) - income (dbl) - experience (dbl) <br>
...
X-label: ‘Experience’ - Title: ‘Histogram of Experience’ <br>
...
![](basics0_files/figure-gfm/unnamed-chunk-25-1.png)<!-- -->
...
![](basics0_files/figure-gfm/unnamed-chunk-26-1.png)<!-- -->
